...
contract PublicWall {
    ...
    struct Post {
        ...
        bool isDeleted;
        // NEW FIELDS
        Comment[] comments; // Array to hold all comments for this post
        mapping(address => bool) reacted; // Simple reaction: tracks if an address has liked/reacted once
        uint256 reactionCount; // Counter for total reactions
    }
    ...
    event CommentAdded(uint256 indexed postIndex, address indexed author, uint256 timestamp, string text); // NEW
    event ReactionToggled(uint256 indexed postIndex, address indexed user, bool added); // NEW
